...
def process_audio(shared_audio, shared_time, shared_pos, lock):
    ...
    p = pyaudio.PyAudio()
    stream = p.open(format=AUDIO_FORMAT, channels=1, rate=SAMPLE_RATE, input=True, frames_per_buffer=CHUNK_SIZE, input_device_index = 2)
...
def process_requests(shared_audio, shared_time, shared_pos, lock):
    ...
    while True:
	    ...
	    try:
                ...
                print ("Exception in baby quiet for:")
        else:
